...
type TSpecialBoxConianerProps = Pick<IPostExtra , 'specialBox'> & {
  mobile ?:boolean
}


export default function SpecialBoxConianer({ specialBox , mobile }: TSpecialBoxConianerProps) {
  return (
          ...
          <div key={item.id} className="space-y-4" id={`spacial_${item.id}`}>
            ...
            {(() => {
              switch (item.content_type) {
                ...
                case "category":
                  return arrayVisible && <SpecialCategoryContainer {...item} mobile={mobile} />;

                case "faq":
                  return arrayVisible && <Accordion {...item} />;

                case "custom_brand":
                  return arrayVisible && <Brands {...item} mobile={mobile} />;
                ...
                case "post":
                  return arrayVisible &&  <Article {...item} mobile={mobile} />;

                case "product":
                  return arrayVisible &&  <SpecialCardContainer {...item} mobile={mobile} />;
                ...
                case "video":
                  return arrayVisible && <SpecialVideo {...item} mobile={mobile}/>;

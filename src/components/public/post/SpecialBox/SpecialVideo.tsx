...
const MySwiperSimple = dynamic(() => import("@/components/public/Swiper"), {
  ssr: false,
  loading: () => (
...
const MySwiperInsta = dynamic(() => import("@/components/public/Swiper"), {
  ssr: false,
  loading: () => (
      <Skeleton height={114} width='100%' />
  ),
});


type TSpecialVideoProps =Pick<SpecialBox, "items" | "show_mode"> &{
  mobile ?: boolean
}
  ...
  return (
    <div>
      {show_mode === 0 ? (
        ...
        <MySwiperInsta slides={videos.map((v) => v.node)} col={mobile ? 3.5 :9} gap={15} />
      )}
